...
fn should_open_proposal() {
	new_test_ext().execute_with(|| {
		...
		// can't reopen
		assert_noop!(GovModule::open_next(Origin::signed(10)), Error::<Test>::UncloseProposal);
	});
}
...
fn should_close_proposal() {
	new_test_ext().execute_with(|| {
		assert_noop!(GovModule::over(Origin::signed(10)), Error::<Test>::UnExistedProposal);
		assert_ok!(GovModule::open_next(Origin::signed(10)));
		assert_noop!(GovModule::over(Origin::signed(10)), Error::<Test>::UnfinishedProposal);
		fast_forward_to(5);
...
fn vote_ok_and_lock_balance() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(GovModule::vote(Origin::signed(10), vt1), Error::<Test>::InsufficientFunds);
		let vt2 = GovVote{
			aye: true,
			balance: 50,
		};
		...
		// can not revote
		assert_noop!(GovModule::vote(Origin::signed(10), vt2.clone()), Error::<Test>::AlreadyVetoed);
...
fn close_proposal_and_unlock_balance() {
	new_test_ext().execute_with(|| {
		assert_ok!(GovModule::open_next(Origin::signed(10)));
		// vote aye
		assert_ok!(GovModule::vote(Origin::signed(10), Vote{ aye: false, balance: 50}));
		assert_noop!(
			Balances::transfer(Origin::signed(10), 30, 1),
			BalancesError::<Test, _>::LiquidityRestrictions
		);
		// vote nay
		assert_ok!(GovModule::vote(Origin::signed(20), Vote{ aye: false, balance: 50}));
		assert_noop!(
			Balances::transfer(Origin::signed(10), 30, 1),
			BalancesError::<Test, _>::LiquidityRestrictions
		);
...
fn pass_proposal_and_can_open_next() {
	new_test_ext().execute_with(|| {
		assert_ok!(GovModule::open_next(Origin::signed(10)));
		assert_ok!(GovModule::vote(Origin::signed(10), Vote{ aye: true, balance: 50}));
		fast_forward_to(5);
		assert_ok!(GovModule::over(Origin::signed(10)));
