...
use node_runtime::{
	...
	TechnicalCommitteeConfig, EVMConfig, IssueConfig, wasm_binary_unwrap,
};
...
fn get_properties() -> Option<Properties> {
	...
	properties.insert("tokenSymbol".into(), "AOC".into());
...
pub fn testnet_genesis(
	...
	GenesisConfig {
		...
		pallet_issue: Some(IssueConfig {
			stgs: vec![(b"step0".as_ref().into(), 10_000_000 * AOC),
					   (b"step1".as_ref().into(), 20_000_000 * AOC),
					   (b"step2".as_ref().into(), 30_000_000 * AOC),
					   (b"step3".as_ref().into(), 40_000_000 * AOC),
					   (b"step4".as_ref().into(), 100_000_000 * AOC),
					   (b"step5".as_ref().into(), 100_000_000 * AOC),
					   (b"step6".as_ref().into(), 180_000_000 * AOC),
					   (b"step7".as_ref().into(), 200_000_000 * AOC)]
		}),
	}
}
